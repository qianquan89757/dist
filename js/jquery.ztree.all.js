...
(function ($) {
    var settings = {}, roots = {}, caches = {},
      ...
      data = {
        ...
        searchNodesByParamFuzzy: function (setting, nodes, key, value) {
          if (!nodes || !key) return [];
          var result = [];
          value = value.toLowerCase();
          for (var i = 0, l = nodes.length; i < l; i++) {
            ...
            if (typeof node[key] == "string" && nodes[i][key].toLowerCase().indexOf(value) > -1) {
              ...
              var children = data.nodeChildren(setting, node);
              result = result.concat(data.searchNodesByParamFuzzy(setting, children, key, value));
            }
          }
          return result;
        },
    ...
    $.fn.zTree = {
      ...
      init: function (obj, zSetting, zNodes) {
        ...
        var zTreeTools = {
          ...
          searchNodesByParamFuzzy: function (key, value, parentNode) {
            if (!key) return null;
            return data.searchNodesByParamFuzzy(setting, parentNode ? data.nodeChildren(setting, parentNode) : data.getNodes(setting), key, value);
          },
    ...
    var _consts = {
      ...
      _beforeA = function (setting, node, html) {
        if (setting.check.enable) {
          ...
          html.push("<span ID='", node.tId, consts.id.CHECK, "' class='", view.makeChkClass(setting, node), "' treeNode", consts.id.CHECK, (node.nocheck === true ? " style='display:none;'" : " style='display:inline-block;'"), "></span>");
        }
      },
      ...
      _view = {
        ...
        setChkClass: function (setting, obj, node, boolean) {
          if (!obj) return;
          if (node.nocheck === true) {
            obj.hide();
          } else {
            if(boolean != undefined) obj.css('display', 'inline-block')
            obj.show();
          }
