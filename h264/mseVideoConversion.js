...
class MseVideoConversion {
  ...
  createWebAssemblyObj () {
    ...
    if (typeof WebAssembly !== 'undefined') {
      ...
      console.log('浏览器版本太低，无法打开音频功能')
      //this.$message.error("'浏览器版本太低，无法打开音频功能'");
    }
  ...
  createMediaSource () {
    ...
    if (this.options.domId !== '') {
       ...
       */
	   /**
	   *nowVideo ==video标签 
	   *this.options.id ==videoId
	   */
    if(this.options.video.parentNode){
      let newVideo = document.createElement('video');
      newVideo.id = this.options.domId;
      newVideo.style.width = '100% !important'; //设置宽高
      newVideo.style.height = '100% !important';//设置宽高
      newVideo.style.objectFit='fill';
      newVideo.autoplay = 'autoplay';//设置播放器属性
      newVideo.muted = "muted";
      // var nowVideo = document.getElementById(this.options.domId);
      // console.info(this.options.video);
      // console.info("--------this.options.video-----");
      var nowVideo = this.options.video;
      // console.info(nowVideo)
      // console.info(newVideo)
      // console.info(nowVideo.parentNode)
    ...
    // this.mediaElement =  this.options.video;
      ...
      this.mediaElement = document.getElementById(this.options.domId);
      // console.info(document.getElementById(this.options.domId))
      ...
      this.mediaDome = this.mediaElement
      /**
       * 给video标签绑定一系列监听事件
       * playing 在媒体开始播放时触发
       * pause 播放暂停时触发
   ...
   */
  fastSlow(val){
    this.mediaElement.playbackRate = val;
    //console.log("vedio倍速为", this.mediaElement.playbackRate);
  }
  ...
  visibilitychangeFun () {
    ...
    if (!isHidden) {
      ...
      //  console.info(this.mediaElement)
      // // if (this.mediaDome) {
      //   if (!this.sourceBuffer.updating) {
 ...
 createOrConnectWebSocket(){
  ...
  websocketInit() {
    ...
    if ('WebSocket' in window && this.options.url !== '') {
      ...
      // console.log("播放器websocket连接开始 " + (new Date()).valueOf());
      /**
       * 设置接收的二进制数据格式
       * @type {string}
       */
  ...
  socketOpen () {
    ...
    // console.log("播放器websocket连接成功 " + (new Date()).valueOf());
  ...
  socketMessage (event) {
    ...
    var flowEndTime = new Date();
    var diffTimes = flowEndTime.getTime()-this.flowStartTime.getTime();
    this.flowByteLength += event.data.byteLength;
   if(diffTimes > 2000){
    var speed = this.flowByteLength / (diffTimes/1000) / 1024;
    //console.log("speed=", speed);
    if(speed > 1024){
      this.flowSpeed = (speed / 1024).toFixed(1)+'MB/s';
    }else{
      this.flowSpeed = speed.toFixed(1)+'KB/s';
    }

    //console.log("speed=", this.flowSpeed);

    this.flowStartTime = flowEndTime;
    this.flowByteLength = 0;
   }
  ...
  // 关闭socket
  closeSocket () {
    //console.info('video socket colose');
    if (this.websocket) {
      this.websocket.close();
      this.socketState = false;
      clearTimeout(this.timeOutMonitoring);
  ...
  monitoringMessage () {
    ...
    this.timeOutMonitoring = setTimeout(function () {
      if (!that.messageState) {
        console.info("monitoringMessage...")
